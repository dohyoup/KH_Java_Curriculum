...
public class Sample07 {
	...
	public void ex01( ) {
		 ...
		 */
		 ...
		 //2
		 SimpleDateFormat cFormat = new SimpleDateFormat("yyyy년 MM월 dd일");
		 String ctrDate = cFormat.format(new Date());
		 System.out.println(ctrDate);
		 //3
		 SimpleDateFormat vFormat = new SimpleDateFormat("M월 d일");
		 String vtrDate = vFormat.format(new Date());
		 System.out.println(vtrDate);
		 //4
		 sFormat = new SimpleDateFormat("a hh시 mm분 E요일");
	...
	public void ex02() {
		/*
		 * 사용자 입력을 이용하여 현재 날짜에서 n일 을 추가하거나 뻈을 때의
		 * 날짜가 언제인지를 확인할 수 잇게 출력하시오.
		 *  예) 
		 *  	일자 : 10
		 *  	현재 날짜에서 10일 후 날짜는 2022년 04월 11일 입니다.
		 *  
		 *  	일자 : -10
		 *  	현재 날짜넹서 10일 전 날짜는 2022년 03월 22일 입니다.
		 */
	...
	public void ex04() {
		...
		String input = "";
		while(true) {
		System.out.println("자신의 생일을  \"월.일\" 형식으로 입력하세요.");
		input = sc.nextLine();
		
		if(input.matches("\\d{1,2}\\.\\d{1,2}")) {// {1,2} 는 1~2라는 의미 00.00형식으로 작성하라는 뜻
			break;
		}
		...
		System.out.println(now.after(birthDay)); //전후 날짜 비교
	...
	public void ex06() {
		...
		for(int year = 2020; year < 2100; year++) {
			if(gc.isLeapYear(year)) { //isLeapYear(); 메소드는 해당연도가 윤년이면 트루값 아니면 펄스값을 출력하는 메소드이다.
	...
	public void ex08() {
		...
		GregorianCalendar upload = new GregorianCalendar(2022, 3 - 1 , 28, 12, 46, 20);
		Date now = new Date();
		...
		}else if(beforeTime <= 60 * 60 * 24 * 7 * 7 * 4){
			System.out.println(beforeTime / (60 * 60 * 24 * 7) + " 주 전");
		}else if(beforeTime <= 60 * 60 * 24 * 7 * 7 * 4 * 12){
			System.out.println(beforeTime / (60 * 60 * 24 * 7 * 4) + " 개월 전");
		}else {
			System.out.println(beforeTime / (60 * 60 * 24 * 7 * 4 * 12) + " sus 전");

...
public class EmpBoardDAO {
	...
	public boolean updateStaticsLike(EmpBoardStaticsDTO staticsData) {
	...
	public List<EmpBoardDTO> searchData() {
		String mapperId = String.format(mapper, "searchData");
		List<EmpBoardDTO> data = session.selectList(mapperId);
		return data;
	}
	
	public boolean deleteStaticsData(EmpBoardStaticsDTO data) {
		//유효성위배를 방지하기위해 같이 지워준다.

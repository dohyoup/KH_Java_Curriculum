...
<body>
	...
	<p>
		...
		왜냐하면 하나의 페이지에 HTML, CSS, JS, JSP, JAVA, SQL 등의 코드가 섞여(코드의 스파게티화) 있기 때문에<br>
		로직을 파악하는 것이 어렵다.<br>
		또한 협업은 원활히 이루어 질 수 없다.(사실상 협업 불가능)
	</p>
	<hr>
	<h2>MVC Model(Model 2)</h2>
	<p>
		Model 1 에서 분리가 안된 코드들을 기능별/역할별 코드를 분리하여 관리하는 방식<br>
		새로운 기능/역할 추가가 용이해 진다. 협업, 유지보수 또한 원활해 진다.<br>
	</p>
	<ul>
		...
		<li>흐름 : Request - > Controller -> Model -> Controller -> View -> Controller -> Response </li>
	</ul>
	<hr>
	...
	<ol>
		...
		<li>DeptService 에서는 dept.model.DeptDAO 를 사용하여 직접 데이터베이스에 조회 요청을 하게 한다.</li>
		<li>DeptDAO 는 마이바티스를 이용해서 데이터베이스 연결 세션을 생성 후 사용하게 된다.</li>
		<li>모든 처리 결과는 요청의 역순으로 결과갑을 리턴하게 하여 DeptController 까지 전달한다.</li>

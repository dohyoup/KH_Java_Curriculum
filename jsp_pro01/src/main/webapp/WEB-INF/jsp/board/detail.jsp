...
<body>
	...
	<script type="text/javascript">
		function changeEdit(element) {
			element.innerText = "확인";// 수정버튼 확인으로 변경
			element.nextElementSibling.remove();//삭제버튼 삭제
			
			var value = element.parentElement.previousElementSibling.innerText;//p태그의 inner값 value에 저장
			var textarea = document.createElement("textarea");//textarea태그 생성
			textarea.setAttribute("class", "form-control"); //부트스트랩 양식 불러오기
			textarea.value = value;//textarea 값에 p태그 값 저장
			
			element.parentElement.previousElementSibling.innerText = "";// p태그 값은 빈값으로 만들어준다.
			element.parentElement.previousElementSibling.append(textarea);//p태그 안에 textarea태그 추가

...
public class JoinView {
	...
	public void loginMenu() {
		...
		if(account != null) {
			System.out.println(account.getUserid() + "님이 로그인 하셨습니다.");
	...
	public void afterLoginMenu(JoinVO account) {
		while(true) {
				...
				case "1":
					...
					String sDate = dateFormat.format(createDate); //방법 1
					
					System.out.println(sDate); 
					
					java.util.Date now = new java.util.Date();
					java.util.Date sqlDate = new java.sql.Date(now.getTime()); // 방법 2
					
					sqlDate = java.sql.Date.valueOf("2022-05-13"); // 문자열넣으면 sql데이터로 변환시켜줌 방법3
